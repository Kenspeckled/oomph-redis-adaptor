  ...
  writeAttributes = function(props) {
    ...
    writePromise = idPromise.then(function(id) {
      ...
      for (attr in ref) {
        ...
        switch (obj.dataType) {
          case 'integer':
            if (storableProps[attr] || storableProps[attr] === 0) {
              props[attr] = parseInt(props[attr]);
          ...
          case 'float':
            if (storableProps[attr] || storableProps[attr] === 0) {
              props[attr] = parseFloat(props[attr]);
              storableProps[attr + '[f]'] = props[attr];
              delete storableProps[attr];
            }
            break;
          case 'boolean':
            if (storableProps[attr] != null) {
              props[attr] = props[attr] === 'false' ? false : !!props[attr];
      ...
      for (attr in ref) {
        ...
        switch (obj.dataType) {
          ...
          case 'float':
            ...
            multi.zadd(sortedSetName, value, props.id);
            break;
          case 'string':
            if (obj.sortable) {
              sortedSetName = self.className + ">" + attr;
