...
(function() {
  var removeIndexedSearchableString, sendAttributesForSaving, update,
    slice = [].slice;
  ...
  update = function(id, updateFields, skipValidation) {
    ...
    return getOriginalObjPromise.then(function(originalObj) {
      var attr, i, intersectingValues, len, multiPromise, namespace, newValue, obj, orderedSetName, originalIds, originalValue, ref, remove, removeValue, sortedSetName;
        ...
        if (newValue !== originalValue || _.includes([true, 'true', false, 'false'], newValue) || removeValue) {
          ...
          switch (obj.dataType) {
            ...
            case 'reference':
              ...
              if (obj.many) {
                if (remove) {
                  multi.srem.apply(multi, [self.name + ":" + id + "#" + attr + ':' + obj.referenceModelName + 'Refs'].concat(slice.call(removeValue)));
